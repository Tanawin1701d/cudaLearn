          ...
          )
{
    ...
    //////////
    const int MAX_SUBTILE_ROW = TILE          / SUBTILE;
    const int MAX_SUBTILE_COL = TILE          / SUBTILE;
    const int MAX_ITER        = (N + TILE - 1)/ TILE;
    const int MAX_SUBITER     = TILE          / SUBTILE;
    ...
    for (int iter = 0; iter < MAX_ITER; iter++){   //// TILE LEVEL ITERATION
        ...
        for (int subtile_row = 0; subtile_row < MAX_SUBTILE_ROW; subtile_row++){
            for (int subtile_Col = 0; subtile_Col < MAX_SUBTILE_COL; subtile_Col++){
                ////// TARGET:cast back to tie level indexing
                int tile_smem_des_row = subtile_row * SUBTILE + cell_row;
                int tile_smem_des_col = subtile_Col * SUBTILE + cell_col;
                ////// A: cast back to memory level indexing
                int mem_row_a = tile_mem_srca_row_pivot_start                 + tile_smem_des_row;
                int mem_col_a = tile_mem_srca_col_pivot_start + (iter * TILE) + tile_smem_des_col;
                ////// B: cast back to memory level indexing
                int mem_row_b = tile_mem_srcb_row_pivot_start + (iter * TILE) + tile_smem_des_row;
                int mem_col_b = tile_mem_srcb_col_pivot_start                 + tile_smem_des_col;

                ////// A: row is fixed but col is itering
                bool mem_a_addr_valid = (mem_row_a < M) && (mem_col_a < N);
                A_tile[tile_smem_des_row][tile_smem_des_col] =
                    mem_a_addr_valid ? A[mem_row_a * N + mem_col_a]: 0;
                ////// B: col is fixed but row is itering
                bool mem_b_addr_valid = (mem_row_b < N) && (mem_col_b < K);
                B_tile[tile_smem_des_row][tile_smem_des_col] =
                    mem_b_addr_valid ? B[mem_row_b * K + mem_col_b]: 0;
            }
        }
        __syncthreads();
            ...
            for (int subTile_row = 0; subTile_row < TILE / SUBTILE; subTile_row++){
                for (int subTile_Col = 0; subTile_Col < TILE / SUBTILE; subTile_Col++){
                    ...
                    for (int i = 0; i < SUBTILE; i++){
                        ans_buf[subTile_row][subTile_Col] += A_tile[tile_row_a][tile_col_a + i] *
                                                             B_tile[tile_row_b + i][tile_col_b];
                    }
                }
            }
        }
